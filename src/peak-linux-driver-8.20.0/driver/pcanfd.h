...
/* Constants */
#define PCANFD_CAN20_MAXDATALEN	8
#define PCANFD_CANFD_MAXDATALEN	64
...
#define __flex_array_of_struct(_n)					\
	_n##s_0 {							\
		__u32		count;					\
		struct _n	list[];					\
	}
...
#else
#define __flex_array_of_struct(_n)	__array_of_struct(_n, 0)
#endif
...
#define PCANFD_DLC_MASK		0x00000F00
#define PCANFD_DLC_SHIFT	8
...
#define PCAN_MAXDATALEN		PCANFD_CAN20_MAXDATALEN
#define PCANFD_MAXDATALEN	PCANFD_CANFD_MAXDATALEN
...
struct __flex_array_of_struct(pcanfd_msg);
...
/* filters list base type (flex array of items) */
struct __flex_array_of_struct(pcanfd_msg_filter);
...
/* clocks value list base type (flex array of items) */
struct __flex_array_of_struct(pcanfd_available_clock);
...
/* bittimings ranges list base type (flex array of items) */
struct __flex_array_of_struct(pcanfd_bittiming_range);
...
/* DLC handling */
static inline __u8 pcanfd_msg_read_dlc(struct pcanfd_msg *msg)
{
	return ((msg->flags & PCANFD_DLC_MASK) >> PCANFD_DLC_SHIFT);
}

static inline void pcanfd_msg_store_dlc(struct pcanfd_msg *msg, __u8 dlc)
{
