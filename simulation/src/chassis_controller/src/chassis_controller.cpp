...
{
  // 计算四个轮子的目标速度 (rad/s)
  double linear_x_ = 0.6*vel_msg_.linear.x; 
  double linear_y_ = 0.6*vel_msg_.linear.y;
  double angular_z_ = 0.2*vel_msg_.angular.z;
  double L = wheel_base_;                  // 轴距
  double W = wheel_track_;                 // 轮距
  double R = wheel_radius_;                // 轮半径
